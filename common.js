...
async function logoutUser() { if (supabaseClient) { try { await supabaseClient.auth.signOut(); } catch (e) { console.error(e); } } currentUser=null; currentUserData=null; }

async function saveCurrentUserData() {
  if (!supabaseClient || !currentUserData?.id) return {success:false};
  ...
  const {data,error} = await supabaseClient.from('profiles').update(payload).eq('id', currentUserData.id).select('*').single();
...
async function saveUsersData(usersData) {
  if (!supabaseClient || !isAdmin()) return {success:false};
  for (const [login,data] of Object.entries(usersData)) {
    if (!data.id) continue;
    await supabaseClient.from('profiles').update({balance:data.balance,history:data.history||[],memes:data.memes||[],pending:data.pending||[]}).eq('id',data.id);
  }
